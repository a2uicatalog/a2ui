...
.catalog-explainer{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);box-shadow:var(--shadow);padding:26px 28px;margin-bottom:32px}
.catalog-explainer-kicker{font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:var(--accent);margin-bottom:8px}
.catalog-explainer h2{margin:0 0 10px;font-size:24px;letter-spacing:-.01em}
.catalog-explainer>p{max-width:72ch;margin:0 0 18px;line-height:1.6}
.catalog-explainer-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:18px;margin-bottom:16px}
.catalog-explainer-grid h3{font-size:14px;margin:0 0 6px}
.catalog-explainer-grid p{margin:0;font-size:14px;line-height:1.55;opacity:.88}
.catalog-explainer-foot{margin:0;font-size:14px;font-weight:600}
.catalog-explainer code{font-size:.9em}
...
def _a2ui_catalog_explainer_html():
    """Answers the generic query head-on. "A2UI catalog" is a term from the
    A2UI protocol itself (the set of component types an agent may use,
    identified by a catalogId URL), so the phrase is owned by the spec, not
    by this site -- searchers typing it get the concept, not us. This section
    connects the two in the searcher's own words: what an A2UI catalog is,
    and that THIS site is one (the A2UI Atomic Catalog -- the phrase we
    already rank for, kept deliberately). Every claim below is the real
    discovery flow (list_catalogs -> get_catalog -> render_surface,
    required_catalogs for the reverse lookup) and the real file layout
    (atoms/atom-packs.yaml -> /catalogue/<slug>.json + /catalogue/index.json)."""
    return '''<section class="catalog-explainer" id="what-is-an-a2ui-catalog" aria-labelledby="catalog-explainer-h">
      <div class="catalog-explainer-kicker">A2UI Catalog &middot; The Concept, And This Site</div>
      <h2 id="catalog-explainer-h">What is an A2UI catalog?</h2>
      <p>In the A2UI protocol, a <strong>catalog</strong> is the set of component types an agent is allowed to
      use, identified by a <code>catalogId</code> URL. An agent doesn't invent UI &mdash; it composes from the
      catalog it has been given. This site is exactly that: the <strong>A2UI Atomic Catalog</strong> &mdash; an
      independent, open A2UI catalog of typed atoms, published as plain JSON any agent can resolve.</p>
      <div class="catalog-explainer-grid">
        <div>
          <h3>One base catalog, many extensions</h3>
          <p><code>a2ui-atoms-v1</code> is always resolved &mdash; structure, text, inputs, actions, wired state.
          Extension catalogs (charts, booking, effects, editorial, ops&hellip;) each carry a declared
          <em>when-to-use</em>, so an agent picks by need, not by recall.</p>
        </div>
        <div>
          <h3>Discovered, not memorised</h3>
          <p>Over MCP: <code>list_catalogs</code> &rarr; <code>get_catalog</code> &rarr; <code>render_surface</code>.
          Going the other way, <code>required_catalogs</code> tells you which catalogs a payload depends on.
          Atoms are never registered as individual tools.</p>
        </div>
        <div>
          <h3>Plain files, real URLs</h3>
          <p>Every catalog is a JSON file under <a href="/catalogue/index.json"><code>/catalogue/</code></a>,
          and the index lists them all with their when-to-use text. The same catalog IDs work on the web,
          in MCP Apps, in Google Chat and in Apps Script.</p>
        </div>
      </div>
      <p class="catalog-explainer-foot"><a href="/catalogue/index.json">Read the catalog index &rarr;</a>
      &nbsp;&middot;&nbsp; <a href="/surfaces/mcp-apps">Use it over MCP &rarr;</a></p>
    </section>'''
...
<head>
  ...
  <meta name="theme-color" content="#6366f1">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>A2UI Catalog — The A2UI Atomic Catalog for AI Agents &amp; MCP</title>
  ...
  <meta property="og:title" content="A2UI Catalog — The A2UI Atomic Catalog">
  ...
  <meta name="twitter:title" content="A2UI Catalog — The A2UI Atomic Catalog">
  ...
  {{
    ...
    "@graph": [
      {{
        ...
        "@id": "https://a2uicatalog.ai/#software",
        "name": "A2UI Catalog",
        "alternateName": ["A2UI Atomic Catalog", "a2uicatalog"],
        ...
        "image": "https://a2uicatalog.ai/brand/og-card.png",
        "sameAs": ["https://github.com/a2uicatalog/a2ui"],
        "author": {{ "@id": "https://a2uicatalog.ai/#person" }},
        "publisher": {{ "@id": "https://a2uicatalog.ai/#org" }},
        "offers": {{ "@type": "Offer", "price": "0", "priceCurrency": "USD" }}
      }},
      {{
        "@type": "Organization",
        "@id": "https://a2uicatalog.ai/#org",
      ...
      {{
        "@type": "Person",
        "@id": "https://a2uicatalog.ai/#person",
        "name": "Curtis Krygier",
        "url": "https://www.linkedin.com/in/curtiskrygier",
        "sameAs": [
          "https://www.linkedin.com/in/curtiskrygier",
          "https://github.com/a2uicatalog"
        ]
      }},
      {{
        "@type": "WebSite",
        "@id": "https://a2uicatalog.ai/#website",
        "url": "https://a2uicatalog.ai",
        "name": "A2UI Atomic Catalog",
        "publisher": {{ "@id": "https://a2uicatalog.ai/#org" }},
        "inLanguage": "en",
...
{cards_html}  </div>
  <footer>
    <span>A2UI Atomic Catalog · <a href="/docs/">API Documentation</a> · <a href="/developers/">Developers</a> · <a href="https://github.com/a2uicatalog/a2ui">GitHub</a> · Maintained by <a href="https://www.linkedin.com/in/curtiskrygier" rel="me author">Curtis Krygier</a></span>
    <span>Independent, unofficial catalog — not affiliated with, endorsed by, or sponsored by Google, Anthropic or OpenAI. A2UI is Google's protocol; official spec at <a href="https://a2ui.org">a2ui.org</a>. MCP is Anthropic's protocol; Claude, the Claude logo and Anthropic are trademarks of Anthropic PBC. ChatGPT and OpenAI are trademarks of OpenAI, L.L.C. All other marks belong to their respective owners.</span>
    <span><a href="/.well-known/ard.json">ARD discovery catalog (JSON)</a></span>
...
.try-render-btn{font:inherit;font-size:13px;font-weight:700;color:var(--accent);background:var(--accent-soft-bg);border:none;border-radius:8px;padding:8px 16px;cursor:pointer;letter-spacing:.02em;margin-bottom:14px}
.try-render-btn:hover{filter:brightness(1.05)}
...
(function(){{
  ...
  // Same gzip+base64url #p= encoding already used by the MCP Apps playground
  // (scripts/generate_atom_pages.py's MCP_APPS_PLAY_HTML) — reusing the real
  // renderer instead of building a second one here.
  function openInPlayground(blocks){{
    if (typeof CompressionStream === 'undefined') {{
      alert('Rendering needs a newer browser (CompressionStream API not available).');
      return;
    }}
    // Open the tab SYNCHRONOUSLY, inside the click handler, before any async work —
    // encoding via CompressionStream is async, and window.open() called after an
    // await/.then() loses the user-gesture context in most browsers, which either
    // silently blocks the popup or leaves a blank about:blank tab (reads as a white
    // screen). Opening first and navigating it once the payload is ready avoids that.
    var win = window.open('about:blank', '_blank');
    var stream = new Blob([JSON.stringify(blocks)]).stream().pipeThrough(new CompressionStream('gzip'));
    new Response(stream).arrayBuffer().then(function(buf){{
      var bytes = new Uint8Array(buf);
      var bin = '';
      for (var i = 0; i < bytes.length; i++) bin += String.fromCharCode(bytes[i]);
      ...
      if (win && !win.closed) {{
        ...
        // popup was blocked outright — fall back to same-tab navigation rather
  ...
  function render(data, status){{
    ...
    if (!data.matched || data.matched.length === 0) {{
      ...
      lastBlocks = (data.blocks && data.blocks.length) ? data.blocks : null;
      ...
      if (lastBlocks) {{
        parts.push('<button class="try-render-btn" id="try-render" type="button">Render it →</button>');
      }}
  ...
  result.addEventListener('click', function(e){{
    var b = e.target.closest('#try-render');
    if (!b || !lastBlocks) return;
    openInPlayground(lastBlocks);
  }});
}})();
  </script>
{_cursor_glow_html()}
