...
# atoms_scene_data.gs (the scene kit's generated data, ~677 KB raw) IS bundled since 2026-09-20 (test_bundle_size_guard ceiling
# raised to 3.2 MB with Curtis's go-ahead). If a host turns out to reject a template this size, add it back here: scene_stage /
# clipart then answer with a readable "kit data is not bundled on this surface" card (see a2ui-private/scene-stage-draft/BRIEF.md).
...
(function() {
  ...
  window._A2UI_HOST_BRIDGE = {
    callTool: function (name, args) {
      // Found live 2026-08-06: compose_surface (Composer, on the Workspace host
      // only — see gen_authoring.py's WORKSPACE_HOST_JS special-case) is the
      // one mcp: tools/call action that runs a REAL Gemini generation
      // server-side rather than a deterministic lookup, and it intermittently
      // outlasted the 15s ceiling every other tools/call action here is
      // legitimately fast enough for — the client-side bridge gave up and
      // rejected with "host tools/call timeout" while the host was still
      // waiting on Vertex, even on calls that would otherwise have succeeded.
      // Bumped to the same "this can genuinely take a while" tier sendMessage
      // below already uses (60s for a human consent decision) but longer
      // still, since a compose call can involve a validation-retry round trip
      // (two sequential model calls, not one). A hung/broken call now takes
      // longer to surface as an error, which is the right trade-off — nothing
      // else calling through this bridge should ever legitimately approach it.
