"""signal_tunnel (2026-09-19) — a canvas hero background whose whole config is
baked into inline JavaScript. Sibling of flow_field on the same shared kit
(_A2UI_CANVAS_KIT_JS), so the same two things are worth proving on every run,
against BOTH real renderers (GAS via the same Node-eval harness
...
from renderers.web_article import _render_signal_tunnel  # noqa: E402

UID_RE = re.compile(r"st-[a-z0-9]{6}")
...
FULL = {
    "eyebrow": "Signal", "title": "Every message arrives",
    "body": "Streams **converge** on the surface.",
    "align": "left", "direction": "out", "colors": ["#38BDF8", "#818cf8", "#f472b6"],
...
def test_invalid_colours_are_dropped_not_fatal():
    cfg = _cfg(_render_signal_tunnel({"colors": ["nope", "#00ff00", "#zzzzzz", "#ff0000"]}))
    assert 'pal:["0,255,0","255,0,0"]' in cfg


def test_palette_capped_at_four():
    cfg = _cfg(_render_signal_tunnel({"colors": ["#010101"] * 9}))
    assert cfg.count("1,1,1") == 4


@pytest.mark.parametrize("value,expected", [
    ("huge", "450"), (None, "450"), (12, "450"), (["low"], "450"), ("low", "250"),
])
...
@pytest.mark.parametrize("value,expected", [
    ("sideways", "in"), (None, "in"), (7, "in"), (["out"], "in"), ("out", "out"), ("in", "in"),
])
def test_direction_falls_back_to_default(core_js, value, expected):
...
def test_gas_output_terminates_its_script_safely(core_js):
    html = _render_via_gas(core_js, {"type": "signal_tunnel"})
...
def test_uid_is_content_derived_on_the_python_side():
    a = _render_signal_tunnel({"title": "one"})
    b = _render_signal_tunnel({"title": "one"})
    c = _render_signal_tunnel({"title": "two"})
    assert a == b
