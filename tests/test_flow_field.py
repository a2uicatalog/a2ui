"""flow_field (2026-09-18) — a canvas hero background whose whole config is
baked into inline JavaScript. That makes two things worth proving on every
run, against BOTH real renderers (GAS via the same Node-eval harness
...
from renderers.web_article import _render_flow_field  # noqa: E402

UID_RE = re.compile(r"ff-[a-z0-9]{6}")
...
FULL = {
    "eyebrow": "Agent to UI", "title": "Streams become surfaces",
    "body": "Every message an agent sends **lands** as interface.",
    "align": "left", "focus": "right", "colors": ["#38BDF8", "#818cf8", "#f472b6"],
    "background": "#070a12", "density": "high", "speed": "fast", "trail": "long",
    "scale": "broad", "height": 420, "interactive": False,
}
...
@pytest.mark.parametrize("block", [
    {},
    FULL,
    {"title": "Centered", "align": "center", "focus": "center"},
    {"title": "Right", "align": "right", "focus": "left", "colors": ["#ffffff"]},
])
def test_python_renderer_matches_gas_renderer_exactly(core_js, block):
    gas_html = _render_via_gas(core_js, dict(block, type="flow_field"))
    py_html = _render_flow_field(dict(block))
    assert _norm(gas_html) == _norm(py_html)


def test_script_runs_under_node_syntax_check():
    html = _render_flow_field(FULL)
...
def test_every_field_is_baked_as_declared():
    cfg = _cfg(_render_flow_field(FULL))
    assert cfg == ('{n:1100,spd:1.6,trail:0.035,sc:0.0019,'
                   'pal:["56,189,248","129,140,248","244,114,182"],'
                   'bg:"#070a12",bgRgb:"7,10,18",focus:"right",inter:false}')


def test_defaults_when_nothing_is_set():
    html = _render_flow_field({})
    assert _cfg(html) == ('{n:650,spd:1,trail:0.07,sc:0.0034,'
                          'pal:["56,189,248","129,140,248","244,114,182"],'
                          'bg:"#070a12",bgRgb:"7,10,18",focus:"right",inter:true}')
...
def test_free_form_colour_never_reaches_the_script(core_js, bad):
    ...
    for html in (_render_flow_field(block),
                 _render_via_gas(core_js, dict(block, type="flow_field"))):
...
def test_invalid_colours_are_dropped_not_fatal():
    cfg = _cfg(_render_flow_field({"colors": ["nope", "#00ff00", "#zzzzzz", "#ff0000"]}))
    assert 'pal:["0,255,0","255,0,0"]' in cfg


def test_palette_capped_at_four():
    cfg = _cfg(_render_flow_field({"colors": ["#010101"] * 9}))
    assert cfg.count("1,1,1") == 4


@pytest.mark.parametrize("value,expected", [
    ("huge", "650"), (None, "650"), (12, "650"), (["low"], "650"), ("low", "350"),
])
...
def test_gas_output_terminates_its_script_safely(core_js):
    html = _render_via_gas(core_js, {"type": "flow_field"})
...
def test_uid_is_content_derived_on_the_python_side():
    a = _render_flow_field({"title": "one"})
    b = _render_flow_field({"title": "one"})
    c = _render_flow_field({"title": "two"})
    assert a == b
