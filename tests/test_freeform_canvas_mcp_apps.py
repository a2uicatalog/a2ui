...
def test_completeness_pass_common_presentation_attrs_are_allowed(core_js):
    """Mirrors the Python reference's identical test -- see that file for
    the full story (two unrelated live rejections for ordinary SVG
    presentation attributes -> a completeness pass, not another one-off
    patch)."""
    out = _render_all(core_js, {"case": _base(
...
def test_marker_start_mid_end_allowed_and_id_namespaced(core_js):
    elements = [
        {"tag": "defs", "children": [
...
def test_marker_start_external_url_rejected(core_js):
    out = _render_all(core_js, {"case": _base(
        elements=[{"tag": "line", "x1": 0, "y1": 0, "x2": 1, "y2": 1,
                   "marker_start": "url(http://evil.example/arrow.svg)"}])})["case"]
    assert REJECTED in out
