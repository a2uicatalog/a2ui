...
_RENDERERS['airspace_command_deck'] = function(b) {
  ...
  return breakout + '<style>' +
    ...
    '<div style="position:absolute;bottom:0;left:0;right:0;height:' + tickerH + ';' +
      ...
      '<div style="overflow:hidden;flex:1;">' +
        '<div id="' + uid + 'tkr" style="white-space:nowrap;font-size:' + tickerSize + ';font-weight:700;color:rgba(255,255,255,0.88);' +
          ...
          // Matches the status pill's own hardcoded SIM default (above) — JS
          // (renderTicker) overwrites both together the instant either the
          // data feed or the weather feed answers, so this is a same-tick
          // correct default, never a claim that briefly lies before JS runs.
          '○ SIMULATED — ' + _esc(tickerText) + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;○ SIMULATED — ' + _esc(tickerText) +
        '</div>' +
      '</div>' +
    '</div>' +
    ...
    '<script>(function(){' +
      ...
      // _TKR_LIVE tracks the SAME live/sim signal the status pill shows —
      // the ticker's LIVE/SIMULATED prefix (renderTicker, below) reads it
      // too, so the scrolling marquee can never claim "LIVE" while the pill
      // quietly says SIM. Payload-authored ticker_text stays free-form
      // marquee copy; the prefix is the one claim the renderer itself owns.
      'var _TKR_LIVE=false;' +
      ...
      // Single source of truth for ticker content — called by BOTH the
      // weather update and the data-status flip, so neither can clobber the
      // other's contribution (weather text vs the LIVE/SIMULATED prefix).
      'function renderTicker(){' +
        'var tk=document.getElementById("' + uid + 'tkr");' +
        'if(!tk)return;' +
        'var prefix=_TKR_LIVE?"\\u25cf LIVE — ":"\\u25cb SIMULATED — ";' +
        'var t=prefix+_wi(_TKRT,_wxLast);' +
        'tk.textContent=t+"     "+t;' +
      '}' +
      ...
      'function updateWeather(wx){' +
        ...
        'if(cs)cs.textContent=_wi(_CSUB,wx);' +
        'renderTicker();' +
      '}' +
...
_RENDERERS['playbook'] = function(b) {
  ...
  return '<style>' +
    ...
    'window.' + uid + 'go=function(id){' +
      ...
      'window.scrollTo(0,0);' +
      // Arm auto-advance BEFORE touching history. In a sandboxed iframe with
      // an opaque origin (ChatGPT's MCP Apps host, and any host that omits
      // allow-same-origin) replaceState throws SecurityError -- and the old
      // `if(window.history)` guard did not catch it: history EXISTS, it is
      // the CALL that throws. The throw escaped go() one statement before the
      // timer was armed, so the slide painted correctly and then never
      // advanced. Found 2026-08-01 running the Toulouse ATC playbook in
      // ChatGPT; Claude's per-resource origin permits the write, so no host
      // we had tested could surface it. Navigation must never sit downstream
      // of a cosmetic URL update.
      'if(_dur[id])_tmr=setTimeout(function(){window.' + uid + 'go(_nxt[id]);},_dur[id]);' +
      'try{if(window.history&&window.history.replaceState)window.history.replaceState(null,"","#"+id);}catch(e){}' +
    '};' +
