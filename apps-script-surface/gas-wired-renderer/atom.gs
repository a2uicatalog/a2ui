...
function renderAtoms(blocks, opts) {
  ...
  // property (comma list), else null = ALL packs (the full renderer — unchanged).
  // core + promote (ALWAYS_ON_PACKS) are available regardless. Pack map = PackMap.gs
  // (generated from atoms/atom-packs.yaml).
  var enabledPacks = opts.enabledPacks || null;
  if (!enabledPacks) {
    try {
      var _raw = PropertiesService.getScriptProperties().getProperty('A2UI_ENABLED_PACKS');
      enabledPacks = _raw ? _raw.split(',').map(function(s){ return s.trim(); }).filter(Boolean) : null;
    } catch (e) { enabledPacks = null; }
  }
  function _packEnabled(btype) {
    if (!enabledPacks) return true;                          // no config → all on
    if (typeof ATOM_PACK === 'undefined') return true;       // map absent → fail open
    var pack = ATOM_PACK[btype];
    if (!pack) return true;                                  // unmapped extension → allow
    if (typeof ALWAYS_ON_PACKS !== 'undefined' && ALWAYS_ON_PACKS.indexOf(pack) !== -1) return true;
    return enabledPacks.indexOf(pack) !== -1;
  }
  ...
  for (var i = 0; i < blocks.length; i++) {
    ...
    if (fn && !_packEnabled(btype)) {
      // atom exists but its pack isn't enabled in this (lean) workbench
      parts.push('<!-- a2ui: atom "' + _esc(btype) + '" is in pack "' +
                 _esc(ATOM_PACK[btype]) + '" which is not enabled here -->');
    } else if (fn) {
      try {
        parts.push(fn(block));
      } catch (err) {
...
// Was fully hardcoded light-theme hex — invisible borders and clashing blue
// link text under any dark theme (including the new 'terminal' brand theme).
// Switched to the shared CSS custom properties so it follows whichever theme
// class the page shell applies, the same pattern the AtomPage.html nav bar uses.
_RENDERERS['resources_list'] = function(b) {
  var items = b.items || [];
  var rows = items.map(function(item){
    var size = item.size ? '<span style="font-size:0.75rem;color:var(--muted);">' + _esc(item.size) + '</span>' : '';
    var type = item.type ? '<span style="font-size:0.72rem;background:var(--surface2);padding:2px 6px;border-radius:4px;color:var(--muted);">' + _esc(item.type.toUpperCase()) + '</span>' : '';
    return '<a href="' + _esc(item.url||'#') + '" target="_blank" rel="noopener" style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);text-decoration:none;">' +
...
_RENDERERS['form_input'] = function(b) {
  ...
  return '<style>' +
    ...
    '.fi-wrap-' + uid + ' .fi-label{display:block;font-size:14px;font-weight:600;color:var(--text,#374151);margin-bottom:4px;}' +
    '.fi-wrap-' + uid + ' .fi-input{display:block;width:100%;box-sizing:border-box;border:1.5px solid var(--border,#d1d5db);border-radius:8px;padding:9px 12px;font-size:15px;color:var(--text,#111827);background:var(--bg,#fff);outline:none;transition:border-color 0.15s;}' +
    '.fi-wrap-' + uid + ' .fi-input:focus{border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,0.12);}' +
    '.fi-wrap-' + uid + ' .fi-hint{font-size:13px;color:var(--muted,#6b7280);margin-top:4px;}' +
...
_RENDERERS['form_select'] = function(b) {
  ...
  return '<style>' +
    ...
    '.fs-wrap-' + uid + ' .fs-label{display:block;font-size:14px;font-weight:600;color:var(--text,#374151);margin-bottom:4px;}' +
    '.fs-wrap-' + uid + ' .fs-select-wrap{position:relative;}' +
    '.fs-wrap-' + uid + ' .fs-select{display:block;width:100%;box-sizing:border-box;border:1.5px solid var(--border,#d1d5db);border-radius:8px;padding:9px 36px 9px 12px;font-size:15px;color:var(--text,#111827);background:var(--bg,#fff);appearance:none;-webkit-appearance:none;outline:none;cursor:pointer;transition:border-color 0.15s;}' +
    '.fs-wrap-' + uid + ' .fs-select:focus{border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,0.12);}' +
    '.fs-wrap-' + uid + ' .fs-arrow{pointer-events:none;position:absolute;right:12px;top:50%;transform:translateY(-50%);color:var(--muted,#6b7280);font-size:12px;}' +
    '</style>' +
