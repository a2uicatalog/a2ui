...
_RENDERERS['form_textarea'] = function(b) {
  ...
  // `value` (2026-08-06) — real, editable, submittable pre-fill, distinct
  // from `placeholder`'s greyed-out hint text. Found live: a reader
  // reasonably expected an example shown via `placeholder` to already be a
  // usable value and submitted an empty prompt when it wasn't — placeholder
  // text was never a real value in any browser, but Composer's own example
  // read enough like real content that the distinction wasn't obvious.
  // Content goes BETWEEN the tags (a textarea's actual value), never a
  // `value=""` attribute — `_esc` still applies since this is unescaped
  // user/model-composed text landing inside HTML.
