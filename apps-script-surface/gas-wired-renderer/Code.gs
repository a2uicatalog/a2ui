...
function doGet(e) {
  ...
  if (e && e.parameter && e.parameter.peek_progress) {
    var denied = _builderAccessError_();
    if (denied) return HtmlService.createHtmlOutput('<pre>' + denied + '</pre>');
    var peek = _peekProgress(e.parameter.peek_progress);
    return ContentService.createTextOutput(JSON.stringify(peek, null, 2))
      .setMimeType(ContentService.MimeType.JSON);
  }
  if (e && e.parameter && e.parameter.stall) {
    // Viewable by a signed-in builder (clean URL on the main deployment) OR with a
    // valid API token (anonymous, for the api deployment + verification). Stall data
    // is non-sensitive aggregate (counts only, no identities), but keep it off open URLs.
    var sp = PropertiesService.getScriptProperties();
    var stExpect = sp.getProperty('A2UI_API_TOKEN') || '';
    var stTokenOk = !!stExpect && (e.parameter.token || '') === stExpect;
    if (_builderAccessError_() && !stTokenOk) {
      return HtmlService.createHtmlOutput('<pre>Not authorized — sign in as a builder or pass a valid token.</pre>');
    }
    return _stallPage_(e.parameter.stall);
  }
  if (e && e.parameter && e.parameter.api_setup !== undefined) {
    return _apiSetupPage_(e);
  }
  if (e && e.parameter && e.parameter.debug_t212 !== undefined) {
    var dbgDenied = _builderAccessError_();
    if (dbgDenied) return _errorPage(dbgDenied);
    var dbg = { portfolio: _actionT212Portfolio({}), candles: _actionMarketCandles({ config: { symbol: 'NVDA', outputsize: 3 } }) };
    return HtmlService.createHtmlOutput('<pre style="padding:24px;font-family:monospace;">' +
      JSON.stringify(dbg, null, 1).replace(/</g, '&lt;') + '</pre>')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
  if (_isAnonymousDeployingMode_()) {
    return HtmlService.createHtmlOutput(
      '<div style="font-family:system-ui;max-width:520px;margin:80px auto;padding:24px;' +
      'border:1px solid #e2e8f0;border-radius:12px;color:#1e293b;">A2UI Build API — ' +
      'POST training.md to this URL with ?api=training&token=… . Pages are served on the main deployment.</div>'
    ).setTitle('A2UI Build API').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
  ...
  // 🎓 Graduated (standing) app: a baked home surface (HomeSurface.gs) renders by default —
  // this is what makes a graduated app a real app, not a data-in-URL link.
  if (typeof A2UI_HOME_SURFACE !== 'undefined' && A2UI_HOME_SURFACE) {
    return _renderFromPayload(A2UI_HOME_SURFACE, from || '');
  }
...
function doPost(e) {
  if (e && e.parameter && e.parameter.api === 'training') return _apiTraining_(e);
  if (e && e.parameter && e.parameter.api === 'stall') return _apiStall_(e);
  if (e && e.parameter && e.parameter.api === 'chat_image') return _apiChatImage_(e);
  if (e && e.parameter && e.parameter.api === 'chart_image') return _apiChartImage_(e);
...
// Shared: upload PNG bytes as a Chat attachment + post, using the CURRENT auth
// context's token. In the execute-as-owner API deployment that's the OWNER; via
// google.script.run it's the SIGNED-IN user. UploadAttachment needs USER auth —
// this context provides it. (SA app auth is rejected, so never _chatBotToken_ here.)
function _uploadImageAndPost_(pngBytes, caption, space, filename) {
  var props = PropertiesService.getScriptProperties();
  space = space || props.getProperty('CHAT_SPACE_DEFAULT') || '';
  if (!space) return { ok:false, error:'no space (pass space or set CHAT_SPACE_DEFAULT)' };
  if (space.indexOf('spaces/') !== 0) space = 'spaces/' + space;
  var hdrs = { Authorization: 'Bearer ' + ScriptApp.getOAuthToken() };
  var boundary = 'a2ui_' + Utilities.getUuid();
  var pre  = Utilities.newBlob(
    '--' + boundary + '\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n' +
    JSON.stringify({ filename: filename || 'printed.png' }) + '\r\n' +
    '--' + boundary + '\r\nContent-Type: image/png\r\n\r\n').getBytes();
  var tail = Utilities.newBlob('\r\n--' + boundary + '--\r\n').getBytes();
  var payload = pre.concat(pngBytes).concat(tail);
  var up = UrlFetchApp.fetch(
    ...
    { method:'post', contentType:'multipart/related; boundary=' + boundary,
      payload: payload, headers: hdrs, muteHttpExceptions:true });
  if (up.getResponseCode() !== 200)
    return { ok:false, stage:'upload', code:up.getResponseCode(), error:up.getContentText().slice(0,300) };
  var ref = (JSON.parse(up.getContentText()) || {}).attachmentDataRef || {};
  var mr = UrlFetchApp.fetch('https://chat.googleapis.com/v1/' + space + '/messages',
    { method:'post', contentType:'application/json',
      payload: JSON.stringify({ text: caption || 'Printed image', attachment: [{ attachmentDataRef: ref }] }),
      headers: hdrs, muteHttpExceptions:true });
  if (mr.getResponseCode() !== 200)
    return { ok:false, stage:'message', code:mr.getResponseCode(), error:mr.getContentText().slice(0,300) };
  return { ok:true, message: (JSON.parse(mr.getContentText())||{}).name };
}

// Pattern-10 auth broker: token-gated endpoint. Runs execute-as-OWNER, so the
// upload is owner (user) auth. Caller holds only A2UI_API_TOKEN — no chat scope.
function _apiChatImage_(e) {
  var out = function(o){ return ContentService.createTextOutput(JSON.stringify(o))
                          .setMimeType(ContentService.MimeType.JSON); };
  try {
    var props  = PropertiesService.getScriptProperties();
    var expect = props.getProperty('A2UI_API_TOKEN');
    var token  = (e.parameter && e.parameter.token) || '';
    if (!expect || token !== expect) return out({ ok:false, error:'unauthorized' });
    var body = {};
    if (e.postData && e.postData.contents) body = JSON.parse(e.postData.contents);
    if (!body.png_base64) return out({ ok:false, error:'missing png_base64' });
    var res = _uploadImageAndPost_(Utilities.base64Decode(body.png_base64), body.caption, body.space, body.filename);
    if (res.ok) res.via = 'owner-broker';
    return out(res);
  } catch(err) { return out({ ok:false, error: String(err) }); }
}

// Client capture (html2canvas → google.script.run): the SIGNED-IN user's browser
// rasterises ANY atom to PNG; this runs as that user (user auth) and posts as them.
// The "any atom → Chat alert image, entirely in-app, no chromium" path.
function captureImageToChat(pngB64, caption, space) {
  try {
    var res = _uploadImageAndPost_(Utilities.base64Decode(pngB64), caption, space, 'atom.png');
    if (res.ok) res.via = 'client-capture';
    return res;
  } catch(err) { return { ok:false, error: String(err) }; }
}

// GAS-NATIVE render: the built-in Charts service rasterises a chart to PNG on the
// SERVER — no chromium, no browser. Covers the chart family (line/column/bar/area/
// pie/scatter). spec = {type, title, labels, datasets|series | data}.
function _chartServicePng_(spec) {
  var type = String(spec.type || spec.chart || 'column').toLowerCase();
  var dt   = Charts.newDataTable();
  var chart;
  if (type.indexOf('pie') >= 0) {
    dt.addColumn(Charts.ColumnType.STRING, 'Label').addColumn(Charts.ColumnType.NUMBER, 'Value');
    (spec.data || []).forEach(function(d){ dt.addRow([String(d.label), Number(d.value)]); });
    chart = Charts.newPieChart();
  } else {
    var labels = spec.labels || [];
    var series = spec.datasets || spec.series || [];
    dt.addColumn(Charts.ColumnType.STRING, spec.label_x || 'X');
    series.forEach(function(ds, i){ dt.addColumn(Charts.ColumnType.NUMBER, ds.label || ('S' + (i+1))); });
    labels.forEach(function(lab, r){
      var row = [String(lab)];
      series.forEach(function(ds){ row.push(Number((ds.data || [])[r] || 0)); });
      dt.addRow(row);
    });
    if (type.indexOf('line') >= 0)       chart = Charts.newLineChart();
    else if (type.indexOf('area') >= 0)  chart = Charts.newAreaChart();
    else if (type.indexOf('scatter') >= 0) chart = Charts.newScatterChart();
    else if (type.indexOf('bar') >= 0 && type.indexOf('column') < 0) chart = Charts.newBarChart();
    else                                  chart = Charts.newColumnChart();
  }
  return chart.setDataTable(dt).setTitle(spec.title || '').setDimensions(640, 340)
              .build().getAs('image/png').getBytes();
}

// Token-gated GAS-native chart -> Chat. Renders with the Charts service (server-
// side, chromium-free) then posts via the owner broker. body = {chart:{...}, caption, space}.
function _apiChartImage_(e) {
  var out = function(o){ return ContentService.createTextOutput(JSON.stringify(o))
                          .setMimeType(ContentService.MimeType.JSON); };
  try {
    var props  = PropertiesService.getScriptProperties();
    ...
    var body = {};
    if (e.postData && e.postData.contents) body = JSON.parse(e.postData.contents);
    var spec = body.chart || body;
    var png  = _chartServicePng_(spec);
    var res  = _uploadImageAndPost_(png, body.caption, body.space, (spec.type || 'chart') + '.png');
    if (res.ok) res.via = 'gas-charts-service';
    return out(res);
  } catch(err) { return out({ ok:false, error: String(err) }); }
}
...
function a2uiAction(type, payload) {
  ...
  try {
    switch (type) {
      ...
      case 'gas:t212_portfolio':     result = _actionT212Portfolio(payload);    break;
      case 'gas:market_candles':     result = _actionMarketCandles(payload);    break;
...
// _WIRED_ATOM_ALIASES / _resolveInitialRows / the layout loop moved to
// atoms_wired_render.gs (wired-transport v0.1, 2026-07-11) — one renderer,
// both surfaces; GAS merges .gs files so behavior is unchanged.
function _renderWiredSurface(payload, navSlug) {
  ...
  var content    = _a2uiRenderWiredLayout(payload);
...
function _renderNamedPage(slug, from) {
  ...
  try {
    ...
    var payload = JSON.parse(json);
    if (!Array.isArray(payload) && payload.type === 'a2ui_wired_surface') {
      return _renderWiredSurface(payload, slug);
    }
    var blocks  = Array.isArray(payload) ? payload : (payload.blocks || []);
...
function _publicBaseUrl_() {
  // The API deployment's getUrl() points at itself; persisted links must use
  // the public deployment. Captured by ?api_setup=1 run on the public URL.
  // Normalize away the Workspace domain-scoped path (/a/<domain>/) so links
  // are shareable outside the domain session that captured them.
  var url = PropertiesService.getScriptProperties().getProperty('A2UI_PUBLIC_URL') || _getWebAppUrl();
...
function _actionBuildTraining(payload) {
  ...
  var res = _buildTrainingCore_(md);
  if (!res.ok) return { ok: false, error: res.error };
  var summary = res.url + '   —   ' + res.coverage;
...
function _buildTrainingCore_(md) {
  ...
  var url = _publicBaseUrl_() + '?nav=' + slug;
  return { ok: true, url: url, slug: slug,
           coverage: res.report.coverage, steps: res.report.step_count,
...
// ─── Build API — token-gated programmatic training.md -> app ─────────────────
// Served by a dedicated deployment (executeAs: owner, anonymous access).
// Session identity is unavailable there, so the gate is a shared token in
// Script Properties (A2UI_API_TOKEN), minted via ?api_setup=1 by a builder.

function _isAnonymousDeployingMode_() {
...
function _apiTraining_(e) {
  function json(obj) {
    return ContentService.createTextOutput(JSON.stringify(obj))
      .setMimeType(ContentService.MimeType.JSON);
  }
  var expected = PropertiesService.getScriptProperties().getProperty('A2UI_API_TOKEN') || '';
  if (!expected) return json({ ok: false, error: 'API disabled: A2UI_API_TOKEN not configured (visit ?api_setup=1 on the main deployment)' });
  var got = (e.parameter && e.parameter.token) || '';
  if (got !== expected) return json({ ok: false, error: 'invalid token' });
  var md = (e.postData && e.postData.contents) || '';
  return json(_buildTrainingCore_(md));
}

// Builder-gated token management. Run on the MAIN deployment while signed in:
// mints the token if unset (?api_setup=1), rotates on ?api_setup=rotate, and
// records this deployment's URL as A2UI_PUBLIC_URL for persisted links.
function _apiSetupPage_(e) {
  var denied = _builderAccessError_();
  if (denied) return _errorPage(denied);
  var props = PropertiesService.getScriptProperties();
  props.setProperty('A2UI_PUBLIC_URL', _getWebAppUrl().replace(/\/a\/[^\/]+\//, '/'));
  var mode = (e.parameter && e.parameter.api_setup) || '1';
  var token = props.getProperty('A2UI_API_TOKEN') || '';
  var msg;
  if (!token || mode === 'rotate') {
    token = Utilities.getUuid().replace(/-/g, '') + Utilities.getUuid().replace(/-/g, '');
    props.setProperty('A2UI_API_TOKEN', token);
    msg = 'API token ' + (mode === 'rotate' ? 'rotated' : 'created') + '. Store it now — it is shown once:' +
      '<div style="font-family:monospace;background:#f1f5f9;padding:12px;border-radius:8px;' +
      'margin:12px 0;word-break:break-all;">' + token + '</div>';
  } else {
    msg = 'API token already configured (…' + token.slice(-6) + '). ' +
      'Append &api_setup=rotate to rotate it. A2UI_PUBLIC_URL refreshed.';
  }
  return HtmlService.createHtmlOutput(
    '<div style="font-family:system-ui;max-width:560px;margin:80px auto;padding:24px;' +
    'border:1px solid #e2e8f0;border-radius:12px;color:#1e293b;">' + msg +
    '<div style="font-size:0.85rem;color:#64748b;margin-top:12px;">Usage: POST training.md as the request body to ' +
    'the API deployment /exec?api=training&token=&lt;token&gt;</div></div>'
  ).setTitle('A2UI API Setup').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}


// ─── Trading data actions (read-only observation plane) ──────────────────────
// Keys live in Script Properties (T212_API_KEY, TWELVE_API_KEY) — default deny.
// T212_BASE_URL defaults to the DEMO account; live requires an explicit property.

function _actionT212Portfolio(payload) {
  var props = PropertiesService.getScriptProperties();
  var key    = (props.getProperty('T212_API_KEY') || '').trim();
  var secret = (props.getProperty('T212_API_SECRET') || '').trim();
  if (!key || !secret) return { ok: false, error: 'T212_API_KEY / T212_API_SECRET not configured in Script Properties' };
  var base = props.getProperty('T212_BASE_URL') || 'https://demo.trading212.com/api/v0';
  try {
    var basic = Utilities.base64Encode(key + ':' + secret);
    var resp = UrlFetchApp.fetch(base + '/equity/portfolio', {
      headers: { 'Authorization': 'Basic ' + basic }, muteHttpExceptions: true });
    if (resp.getResponseCode() !== 200) {
      return { ok: false, error: 'Trading212 HTTP ' + resp.getResponseCode() };
    }
    var rows = JSON.parse(resp.getContentText()).map(function(pos) {
      return {
        ticker: pos.ticker,
        qty:    pos.quantity,
        avg:    Math.round(pos.averagePrice * 100) / 100,
        price:  Math.round(pos.currentPrice * 100) / 100,
        ppl:    Math.round((pos.ppl || 0) * 100) / 100
      };
    });
    return { ok: true, data: rows, total: rows.length };
  } catch (e) {
    return { ok: false, error: 'Trading212 fetch failed: ' + (e.message || e) };
  }
}

function _actionMarketCandles(payload) {
  var props = PropertiesService.getScriptProperties();
  var key = (props.getProperty('TWELVE_API_KEY') || '').trim();
  if (!key) return { ok: false, error: 'TWELVE_API_KEY not configured in Script Properties' };
  var config = payload.config || payload.props || {};
  var symbol = config.symbol || '';
  if (!symbol) return { ok: false, error: 'symbol is required' };
  var interval   = config.interval   || '1day';
  var outputsize = config.outputsize || 40;
  try {
    ...
    var resp = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
    var json = JSON.parse(resp.getContentText());
    if (json.status === 'error') return { ok: false, error: 'Twelve Data: ' + json.message };
    var rows = (json.values || []).map(function(v) {
      return { t: v.datetime, close: Math.round(parseFloat(v.close) * 100) / 100,
               high: Math.round(parseFloat(v.high) * 100) / 100,
               low:  Math.round(parseFloat(v.low)  * 100) / 100 };
    }).reverse(); // Twelve Data returns newest-first; rolling windows need ascending
    return { ok: true, data: rows, total: rows.length };
  } catch (e) {
    return { ok: false, error: 'Twelve Data fetch failed: ' + (e.message || e) };
  }
}


// ─── Progress persistence (done-states survive reload) ──────────────────────
// Reuses _resolveIdentity() from the MIM/War-Room session work. Key:
// 'prog_<slug>_<identity>' -> JSON {storeId: value, _u: iso}. Identity is the
// signed-in email; anonymous visitors pass a localStorage clientId. Same
// script-property store the surface-presence molecule uses (psurf_*).
// Scale ceiling: script properties (500KB total / 9KB per value) — fine for
// demo + in-tenant training; migrate to a Sheet if it becomes multi-hundred-user.
function _progressKey_(slug, clientId) {
  var email = '';
  try { email = _resolveIdentity().email || ''; } catch (e) {}
  var id = email || ('c_' + String(clientId || 'anon').replace(/[^a-zA-Z0-9]/g, '').slice(0, 40));
  return 'prog_' + String(slug || '').replace(/[^a-zA-Z0-9-]/g, '') + '_' + id;
}

function a2uiLoadProgress(slug, clientId) {
  try {
    if (!slug) return { ok: true, data: {} };
    var raw = PropertiesService.getScriptProperties().getProperty(_progressKey_(slug, clientId));
...
function a2uiSaveProgress(slug, clientId, storeId, value) {
  try {
    if (!slug || !storeId) return { ok: false, error: 'slug and storeId required' };
    var props = PropertiesService.getScriptProperties();
    var key = _progressKey_(slug, clientId);
    var st = {};
    try { st = JSON.parse(props.getProperty(key) || '{}'); } catch (e) {}
    st[storeId] = value;
    st._u = new Date().toISOString();
    props.setProperty(key, JSON.stringify(st));
    return { ok: true };
  } catch (e) { return { ok: false, error: String(e) }; }
}

// Builder-only admin peek: enumerate persisted progress for a slug across all
// identities (enumeration is script-wide, so it works regardless of who runs
// it — used via `clasp run` to verify server persistence). Returns per-identity
// done-counts without exposing which human (key tail is hashed-ish email).
// Map a page's done-state store ids (s_N) -> human step labels by decoding the
// saved nav payload: each command_step's wire.done is "#s_N.value" and props.label
// is the step. Lets the inspector report WHICH steps, not just how many.
function _stepLabels_(slug) {
  var map = {};
  try {
    var raw = PropertiesService.getScriptProperties().getProperty('nav:' + slug);
    if (!raw) return map;
    var enc = JSON.parse(raw).encoded, padded = enc;
    while (padded.length % 4 !== 0) padded += '=';
    var bytes = Utilities.base64DecodeWebSafe(padded);
    var json = (bytes.length >= 2 && bytes[0] === 31 && (bytes[1] & 0xFF) === 139)
      ? Utilities.ungzip(Utilities.newBlob(bytes, 'application/x-gzip')).getDataAsString()
      : Utilities.newBlob(bytes).getDataAsString();
    (JSON.parse(json).layout || []).forEach(function(el) {
      var done = el.wire && el.wire.done;  // "#s_N.value"
      if (done && el.props && el.props.label) {
        var m = String(done).match(/#(\w+)\./);
        if (m) map[m[1]] = el.props.label;
      }
    });
  } catch (e) {}
  return map;
...
// Ordered step list [{id, label}] in layout order — _stepLabels_ loses order in a
// map, but stall analytics needs the sequence to compute "first incomplete step".
function _stepOrder_(slug) {
  var out = [];
  try {
    var raw = PropertiesService.getScriptProperties().getProperty('nav:' + slug);
    if (!raw) return out;
    ...
    (JSON.parse(json).layout || []).forEach(function(el) {
      ...
      if (done && el.props && el.props.label) {
        var m = String(done).match(/#(\w+)\./);
        if (m) out.push({ id: m[1], label: el.props.label });
      }
    });
  } catch (e) {}
  return out;
...
// Stall analytics (Era-2 row-4 follow-on): aggregate the prog_<slug>_* done-states
// across identities into a completion funnel. For each identity: which steps are
// done, and the FIRST incomplete step (its stall point). Truthful on any N — with
// few identities it just reports the small real numbers, never a fabricated curve.
function _stallAnalytics_(slug) {
  var order = _stepOrder_(slug);
  var idToIdx = {};
  order.forEach(function(s, i) { idToIdx[s.id] = i; });
  var N = order.length;
  var props = PropertiesService.getScriptProperties().getProperties();
  var prefix = 'prog_' + String(slug || '').replace(/[^a-zA-Z0-9-]/g, '') + '_';
  var identities = 0, completedAll = 0;
  var stepCompleted = order.map(function() { return 0; });
  var stalledHere   = order.map(function() { return 0; });
  Object.keys(props).forEach(function(k) {
    if (k.indexOf(prefix) !== 0) return;
    var st = {}; try { st = JSON.parse(props[k]); } catch (e) {}
    identities++;
    var done = {};
    Object.keys(st).forEach(function(kk) {
      if (kk !== '_u' && st[kk] === true && idToIdx[kk] !== undefined) done[idToIdx[kk]] = true;
    });
    for (var i = 0; i < N; i++) { if (done[i]) stepCompleted[i]++; }
    var firstIncomplete = -1;
    for (var j = 0; j < N; j++) { if (!done[j]) { firstIncomplete = j; break; } }
    if (firstIncomplete === -1) completedAll++; else stalledHere[firstIncomplete]++;
  });
  return {
    ok: true, slug: slug, total_steps: N, identities: identities,
    completed_all: completedAll,
    completion_pct: identities ? Math.round(100 * completedAll / identities) : 0,
    steps: order.map(function(s, i) {
      return {
        step: i + 1, label: s.label,
        completed: stepCompleted[i],
        completed_pct: identities ? Math.round(100 * stepCompleted[i] / identities) : 0,
        stalled_here: stalledHere[i]
      };
    })
  };
}

// Glass-box stall page: server-renders the funnel from LIVE prog_ data on every
// request (no client wiring to go stale). Builds a blocks-format payload and runs
// it through the same _renderFromPayload stack every other page uses, so it inherits
// the full atom vocabulary + theme. Truthful on any N — the numbers ARE the store.
function _stallPage_(slug) {
  var s = _stallAnalytics_(slug);
  var blocks = [
    { type: 'subheading', text: 'Runbook stall analytics — ' + slug }
  ];
  if (!s.ok || !s.total_steps) {
    blocks.push({ type: 'body', text: 'No progress data for "' + slug + '" yet. This page renders the moment a real run leaves a done-state in the prog_ store — nothing is simulated.' });
    return _renderFromPayload({ title: 'Stall analytics — ' + slug, theme: 'light', blocks: blocks });
  }
  var nId = s.identities;
  var idWord = nId === 1 ? 'identity' : 'identities';
  // First step with a stall (the funnel's cliff edge).
  var stall = null;
  for (var i = 0; i < s.steps.length; i++) { if (s.steps[i].stalled_here > 0) { stall = s.steps[i]; break; } }

  var funnelSteps = s.steps.map(function(st) {
    return { stage: st.step + '. ' + st.label, value: st.completed };
  });
  var tableRows = s.steps.map(function(st) {
    return {
      step: st.step,
      step_name: st.label,
      reached: st.completed,
      reached_pct: st.completed_pct + '%',
      stalled_here: st.stalled_here > 0 ? '⚠ ' + st.stalled_here : ''
    };
  });

  blocks.push({ type: 'body', text:
    'Read-side funnel over the persisted done-states (prog_ store) — the same store the runbook writes as you tick steps. A step counts as reached when its done-state is true; an identity\'s stall point is its first incomplete step. Recomputed live on every load. ' +
    nId + ' ' + idWord + ' recorded' +
    (nId === 1 ? ' — a single real run so far; the funnel sharpens as more people work through the runbook.' : '.') });
  ...
  if (stall) {
    blocks.push({ type: 'body', text:
      '▶ Stall point: step ' + stall.step + ' — “' + stall.label + '”. ' +
      stall.stalled_here + ' of ' + nId + ' ' + idWord + ' got this far and stopped. That is the step to make easier.' });
  } else if (s.completed_all === nId && nId > 0) {
    blocks.push({ type: 'body', text: '▶ No stalls — every recorded identity completed all ' + s.total_steps + ' steps.' });
  }

  blocks.push({ type: 'divider' });
  blocks.push({ type: 'subheading', text: '② Per-step breakdown' });
  blocks.push({ type: 'data_table_sortable', striped: true,
    columns: ['step', 'step_name', 'reached', 'reached_pct', 'stalled_here'],
    rows: tableRows });

  blocks.push({ type: 'divider' });
  blocks.push({ type: 'body', text:
    'completed_all: ' + s.completed_all + ' / ' + nId + ' (' + s.completion_pct + '%) · total steps: ' + s.total_steps +
    ' · source: agent-authored done-states, never fabricated. Era-2 row-4 follow-on (documents that learn).' });

  return _renderFromPayload({ title: 'Stall analytics — ' + slug, theme: 'light', blocks: blocks });
}

// Token-gated read-only verb (api=stall&slug=...). Same A2UI_API_TOKEN as api=training.
function _apiStall_(e) {
  function json(obj) {
    return ContentService.createTextOutput(JSON.stringify(obj))
      .setMimeType(ContentService.MimeType.JSON);
  }
  var expected = PropertiesService.getScriptProperties().getProperty('A2UI_API_TOKEN') || '';
  if (!expected) return json({ ok: false, error: 'API disabled: A2UI_API_TOKEN not configured' });
  var got = (e.parameter && e.parameter.token) || '';
  if (got !== expected) return json({ ok: false, error: 'invalid token' });
  var slug = (e.parameter && e.parameter.slug) || '';
  if (!slug) return json({ ok: false, error: 'slug required' });
  return json(_stallAnalytics_(slug));
}

function _peekProgress(slug) {
  var props = PropertiesService.getScriptProperties().getProperties();
  var prefix = 'prog_' + String(slug || '').replace(/[^a-zA-Z0-9-]/g, '') + '_';
  var labels = _stepLabels_(slug);
  var out = [];
  Object.keys(props).forEach(function(k) {
    if (k.indexOf(prefix) !== 0) return;
    var st = {};
    try { st = JSON.parse(props[k]); } catch (e) {}
    var doneIds = Object.keys(st).filter(function(kk) { return kk !== '_u' && st[kk] === true; });
    out.push({
      identity: k.slice(prefix.length),
      done: doneIds.length,
      steps_done: doneIds.map(function(id) { return labels[id] || id; }),  // human labels
      updated: st._u || ''
    });
  });
  return { ok: true, slug: slug, entries: out };
}
