/**
 * A2UI Private Renderer — restricted to @krygier.fr accounts
 *
 * Four modes:
...
var _PRIVATE_DOMAIN = 'krygier.fr';

function _checkAccess() {
  var email = Session.getActiveUser().getEmail();
  if (!email || !email.toLowerCase().endsWith('@' + _PRIVATE_DOMAIN)) {
    return HtmlService.createHtmlOutput(
      '<!DOCTYPE html><html><head><meta charset="utf-8">' +
      '<style>body{font-family:system-ui,sans-serif;background:#0f172a;color:#f1f5f9;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}' +
      '.box{text-align:center;padding:48px 32px;max-width:400px}' +
      'h1{font-size:1.5rem;font-weight:800;color:#ef4444;margin:0 0 12px}' +
      'p{color:#94a3b8;line-height:1.7;margin:0}' +
      'small{display:block;margin-top:16px;color:#475569;font-size:0.75rem}</style></head>' +
      '<body><div class="box">' +
      '<h1>Access Restricted</h1>' +
      '<p>This renderer is private to <strong>@' + _PRIVATE_DOMAIN + '</strong> accounts.</p>' +
      '<small>Signed in as: ' + (email || 'unknown') + '</small>' +
      '</div></body></html>'
    ).setTitle('A2UI Private — Access Restricted');
  }
  return null;
}
...
function doGet(e) {
  ...
  return HtmlService.createHtmlOutputFromFile('Index')
    .setTitle('A2UI Private — Page Generator')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
