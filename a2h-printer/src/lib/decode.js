// src/lib/decode.js — the v1.0 payload decode path, extracted rather than
// bundling the full 1.4MB renderers.generated.js (plan decision #4: avoid
// shipping the entire compiled atom-renderer catalog — most of it is
// unrelated to Slack and proprietary rendering logic for surfaces this
// package doesn't touch). This is the ~130-line self-contained block that
// mcp-worker/src/renderers.generated.js's own comment confirms is portable:
// "Pure string/JSON work — no DOM, no GAS services — so the bundle's
// Node-tested core block can execute it." Ported verbatim (var/function
// style preserved rather than modernized, to keep this a faithful copy of
// the audited original, not a rewrite).
//
// Source: renderers.generated.js lines 20706-20839 (_a2uiPtrGet,
// _a2uiResolvePath, _a2uiResolveDynamic, _rehydrateV1Surface) + render.js's
// decodeV1 (line 193) + tools.js's mcpDecode (line 2610).
...
// v1.0 createSurface -> the blocks dialect slack-mapping.js speaks.
export function decodeV1(payload) {
  if (!payload || Array.isArray(payload) || payload.version !== 'v1.0' || !payload.createSurface) {
    return payload;
  }
  const out = _rehydrateV1Surface(payload.createSurface);
  const props = payload.createSurface.surfaceProperties || {};
  if (props.title && !out.title) out.title = props.title;
  if (props.theme && !out.theme) out.theme = props.theme;
  return out;
}

// mcpDecode — urlsafe base64 (padding stripped on encode, restored here) ->
// gunzip -> JSON. CompressionStream/DecompressionStream and atob/btoa are
// standard globals in Node 18+, unchanged from the Workers original.
export async function mcpDecode(blob) {
  const b64 = String(blob).replace(/-/g, '+').replace(/_/g, '/');
